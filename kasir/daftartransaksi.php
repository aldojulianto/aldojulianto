<?php
    include('../session.php');
    include "../koneksi.php";
    $data = mysqli_query($koneksi,"select id_transaksi,count(id_transaksi) as jumlah,sum(total) as total, tanggal_input from transaksi a join barang b on a.id_barcode=b.id_barcode where a.total>=1 group by id_transaksi");
...
<body>
  ...
  ?>
				<tr>
					<th><?php echo $d["id_transaksi"]; ?></th>
					<td><?php echo $d["jumlah"]; ?></td>
                    <td>Rp. <?php echo number_format($d["total"],2,',','.');?></td>
                    <td><?php echo $d["tanggal_input"]; ?></td>
					<td>
						<a class="btn btn-danger" href="detaildaftartransaksi.php?id=<?php echo $d["id_transaksi"]; ?>" role="button">Detail</a>
					</td>
				</tr>
				<?php
  }
